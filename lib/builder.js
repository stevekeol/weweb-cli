...
// 根据全局的.js文件(非app.js和非pages/*.js)生成app-service.js文件
let build = (exports.build = function () {
  buildPromise = Promise.all([loadConfig(), util.globJSfiles()]).then(function (res) {
    let config = res[0]
    let files = res[1].map(f => util.normalizePath(f))
    // let pages = config.pages
    let utils = util.groupFiles(files, config)
    let paths = utils.concat('app.js') // paths中是所有的.js文件（除了pages中各页面的.js文件）
    return Promise.all(
